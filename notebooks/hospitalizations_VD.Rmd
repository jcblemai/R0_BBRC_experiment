...
ggsave(phcfr, filename = "../COVID-pomp/results/figs/VD_hcfr.png", width = 5.5, height = 3.5)
phcfr
```
...
p_age <- hosp_data %>% 
  mutate(outcome = case_when(outcome == "deceased" ~ outcome, T ~ "other"),
         outcome = factor(outcome, levels = c("other", "deceased")),
         icu = case_when(!is.na(icu_in) ~ "ICU", T ~ "no ICU" )) %>% 
  ggplot(aes(x = age, fill = outcome)) +
  geom_histogram() +
  theme_bw() +
  ggthemes::scale_fill_few() +
...
ggsave(p_age, filename = "../COVID-pomp/results/figs/VD_hist_age.png", width = 6.5, height = 2.5)
```
...
p_th2r <- filter(hosp_data, outcome == "discharged", is.na(icu_in)) %>% 
  ggplot() +
  geom_histogram(aes(x = time_hosp, y = ..count..), fill = "gray") +
...
p_t2icu <- hosp_data %>% 
  ggplot() +
  geom_histogram(aes(x = time_to_icu, y = ..count..), fill = "gray") +
...
p_tiuc <- hosp_data %>% 
  filter(icu_state != "") %>% 
  mutate(icu_state = case_when(icu_state == "discharged from ICU" ~ "discharged or deceased", T ~ "in ICU")) %>% 
  mutate(agec = case_when(age > 65 ~ "+65", T~ "18-65")) %>% 
  ggplot() +
  geom_histogram(aes(x = time_icu, y = ..count.., fill = icu_state), position = "dodge") +
  # geom_line(data = ticu2d_dens, aes(x = x, y = y)) +
  theme_bw() +
  scale_fill_manual(values = th_cols) +
  guides(fill = guide_legend(title = "ICU outcome")) +
  # facet_wrap(~agec) +
...
p_t2d <- hosp_data %>%
  filter(deceased) %>% 
  ggplot() +
  geom_histogram(aes(x = time_to_death, y = ..count..), fill = "gray") +
...
p_ticu2d <- hosp_data %>%
  filter(deceased) %>% 
  ggplot() +
  geom_histogram(aes(x = time_icu_to_death, y = ..count..), fill = "gray") +
  # geom_line(data = ticu2d_dens, aes(x = x, y = y)) +
  theme_bw() +
...
p_times <- arrangeGrob(p_th, p_t2d, p_th2r, p_tnoicu2d, p_t2icu, p_tiuc, p_ticu2d, 
                       layout_matrix = rbind(c(1, 1),
                                             c(2, 3),
                                             c(4, 5),
                                             c(6, 7)))

ggsave(p_times, filename = "../COVID-pomp/results/figs/VD_times.png", width = 8, height = 10)
...
dist_params <- tribble(
  ...
  "Time to discharge without ICU", list(th2r_dist),
  "Time hospitalized without ICU", list(tnoicuh_dist),
  "Time to death without ICU", list(tnoicu2d_dist),
  "Time hospitalized with ICU", list(ticuh_dist),
  "Time to death", list(th2d_dist),
  "Time to ICU", list(t2icu_dist),
  "Time in ICU", list(ticu_dist),
  "Time from ICU to discharge", list(ticu2r_dist),
...
kable(dist_params,
      ...
      caption = "Table A.1. Estimated parameters of key hospitalization time distributions",
      col.names = c("", "mean", "sd", "mean (logscale)", "sd (logscale)")) %>% 
  write("../COVID-pomp/results/vd_params_table.tex")
