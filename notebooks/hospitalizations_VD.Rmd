...
# Cantonal data
cantonal_data <- read_csv("../data/ch/covid_19/fallzahlen_kanton_total_csv/COVID19_Fallzahlen_Kanton_VD_total.csv") %>% 
...
th2d_dist <- fitDist(filter(hosp_data, !is.na(time_to_death)), "time_to_death", dist = "lnorm")
...
tnoicu2d_dist <- fitDist(filter(hosp_data, outcome == "deceased", is.na(icu_in)), "time_to_death", dist = "lnorm")
...
dist_params <- tribble(
  ~var, ~mean, ~variance,
  "Time hospitalized", th_dist$estimate[1] , th_dist$estimate[2],
  "Time from hospitalization to ICU", t2icu_dist$estimate[1], t2icu_dist$estimate[2],
  "Time in ICU",  ticu_dist$estimate[1],   ticu_dist$estimate[2],
  "Time from hospitalization to death",th2d_dist$estimate[1], th2d_dist$estimate[2],
  "Time from ICU to death", ticu2d_dist$estimate[1], ticu2d_dist$estimate[2],
  "Time from hospitalization to death without ICU", tnoicu2d_dist$estimate[1], tnoicu2d_dist$estimate[2]
)
