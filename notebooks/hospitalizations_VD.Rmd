...
ticuh_dist <- fitDist(filter(hosp_data, !is.na(icu_in)), "time_hosp", dist = "lnorm")
...
p_th <- hosp_data %>% 
  mutate(hosp_state = case_when(outcome == "hospitalized" ~ outcome,
                                T ~ "discharged or deceased")) %>% 
  ggplot() +
  geom_histogram(aes(x = time_hosp, y = ..density.., fill = hosp_state), position = "dodge") +
...
grid.arrange(p_th, p_t2icu, p_tiuc, p_t2d, p_ticu2d, p_tnoicu2d, ncol=2)
```

```{r dist params}

dist_params <- tribble(
  ~var, ~mean, ~meanlog, ~sdlog,
  "Time hospitalized", mean(th_dist$data), th_dist$estimate[1] , th_dist$estimate[2],
  # "Time survive", mean(th2r_dist$data), th2r_dist$estimate[1] , th2r_dist$estimate[2],
  "Time to discharge no ICU", mean(th2r_dist$data), th2r_dist$estimate[1] , th2r_dist$estimate[2],
  "Time hospitalized no ICU", mean(tnoicuh_dist$data), tnoicuh_dist$estimate[1] , tnoicuh_dist$estimate[2],
  "Time hospitalized with ICU", mean(ticuh_dist$data), ticuh_dist$estimate[1] , ticuh_dist$estimate[2],
  "Time from hospitalization to ICU", mean(t2icu_dist$data), t2icu_dist$estimate[1], t2icu_dist$estimate[2],
  "Time in ICU",  mean(ticu_dist$data),  ticu_dist$estimate[1],   ticu_dist$estimate[2],
  "Time from hospitalization to death",  mean(th2d_dist$data),th2d_dist$estimate[1], th2d_dist$estimate[2],
  "Time from ICU to death",  mean(ticu2d_dist$data), ticu2d_dist$estimate[1], ticu2d_dist$estimate[2],
  "Time from hospitalization to death without ICU",  mean(tnoicu2d_dist$data), tnoicu2d_dist$estimate[1], tnoicu2d_dist$estimate[2]
)
...
kable(dist_params,
      ...
      col.names = c("", "mean", "mean (logscale)", "sd (logscale)")) %>% 
