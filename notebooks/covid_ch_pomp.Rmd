...
ll_to_use <- config$likelihoods$to_plot
...
# write_csv(filterstats %>% filter(var == "Rt"), "../../COVID-19_CHVaud/notebooks/r0.csv")
```
...
The estimate for $R_0$ at the start of the epidemic was of 3.19 (95\% CI, 2.69-3.61).
...
plot_states(filterstats, "Rt", cantons = cantons_to_analyze, fancol = 4, ylabel = "Reproduction number")
```
...
r0_reduction %>% 
  ...
  geom_errorbar(aes(ymin = q025 , ymax = q975), width  = 0) + 
  coord_flip() +
  scale_y_continuous(breaks = seq(2, 12, by = 2), labels = format(date_start  + seq(2, 12, by = 2), "%B %d")) +
  labs(y = bquote('Date at which '*R[0]*' went bellow 1'), x = "canton") +
  theme_bw()
```


## Associations with activity changes

We contrast the estimated changes in $R_0$ with changes in activity-related mobility produced by [Google](https://www.google.com/covid19/mobility/), parsed and made available [here](https://github.com/vitorbaptista/google-covid19-mobility-reports). Changes in activity are expressed as relative changes with respect to a baseline computed as the median over a 5-week period Jan 3–Feb 6, 2020.

Results shown bellow suggests a strong association between $R_0$ and reduction in mobility to transit stations, followed by gorcery and workplace.

```{r covar, fig.height=6, fig.width=8, fig.cap="**Figure 4. Association between changes in activity-related mobility and $R_0$.** Changes are given in terms of reductions, therfore negative numbers indicate increases in activity."}
gdata <- read_csv("../data/ch/google_mobility.csv")
...
gdata <- gdata %>% 
  ...
  left_join(cantons_dict, by = c("subregion" = "name")) %>% 
  select(-region, -subregion, -updated_at) %>% 
  gather(var, value, -ShortName)

covar_data <- r0_reduction %>% 
  filter(var == "r0change") %>% 
  ungroup() %>% 
  select(-var) %>% 
  mutate(r0change = 1-mean) %>% 
  inner_join(gdata, by = "ShortName")

covar_sign <- lapply(unique(covar_data$var),
                     function(x) {
                       fit <-lm(r0change ~ I(-value), data = filter(covar_data, var == x)) %>% 
                         summary() 
                       
                         data.frame(coef = fit$coefficients[2, 1],
                                    pval = fit$coefficients[2, 4],
                                    rsqrd = fit$adj.r.squared) %>% 
                         mutate(var = x)
                     }) %>% 
  bind_rows()# %>% 
  # mutate(label = str_c("slople=", format(coef, digits = 2), "\n(p-val ", formatC(pval, format = "e", digits = 1), ", R^2 ", format(rsqrd, digits = 2), ")")) 
  
p_covar <- covar_data %>% 
  ggplot(aes(x = -value * 100, y = r0change * 100)) + 
  geom_smooth(method = lm, alpha = .3) +
  geom_label(aes(label = ShortName), alpha = .6) +
  # geom_abline(aes(intercept =0, slope =1)) +
  facet_wrap(~var, scales = "free_x") +
  labs(x = "Percent reduction in activity", y = "Mean percent reduction in R0") +
  theme_bw() 
...
covar_sign %>% 
  kable(booktabs = T,
        digits = 2,
        caption = "Table 1. Linear regression results."#,
...
states_to_plot <- c("a_D", "H_curr", "U_curr")
plot_cnt_all(filterstats = filterstats, data = data, states_to_plot =  states_to_plot, cantons_to_analyze, ylabel = " ")
```
...
r0_reduction %>% 
  ...
  select(-ll_comp)  %>% 
   formatTableContent(col_order) %>% 
    kable(booktabs = T,
          full_width = T,
          align = "c",
          col.names = c("", rep(c("median", "95% QR"), 2))) %>%
...
### Parameters
We estimate four parameters to available cantonal level data (Table 1). The main parameter of interest is the basic reproduction number $R_0$. We here allow the value of the parameter to change in time to capture behavioral changes following the successive application of non-pharmaceutical interventions (NPIs) by the cantonal and federal governments between March 1st and March 20th. The duration in the infectious state was derived from an assumed generation time of 5.2 days (Ghani et al. 2020), and a latent duration of 4.2 days (i.e. 1 day of infectious but non-symptomatic interval), thus yielding $1/\gamma = 2(5.2 - 4.2) = 2days$.

```{r params, echo=FALSE, message=FALSE, warning=FALSE}
library(yaml)
parameters <- as.data.frame(yaml::read_yaml("../COVID-pomp/data/parameters_v2.yaml"))
ncomps <- tribble(
  ~parameter, ~ncomp,
  "e2i", 1,
  "i2o", 1,
...
# gather(parameters, parameter, value) %>% 
#   left_join(ncomps) %>% 
