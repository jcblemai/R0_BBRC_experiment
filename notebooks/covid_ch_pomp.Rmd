...
filterstats <- readRDS("../COVID-pomp/results/filtered_states_all.rds") %>%
  filter(ll_comp == ll_to_use, ShortName %in% config$places)
...
```{r load data}
# Get data
fdata <- list.files(path = "../data/ch/cases/covid_19/fallzahlen_kanton_total_csv/", pattern = "COVID19_Fallzahlen_Kanton_*", full.names = TRUE)
data <- fdata %>%
...
library(yaml)
parameters <- as.data.frame(yaml::read_yaml("../COVID-pomp/data/parameters.yaml"))
ncomps <- tribble(
  ~parameter, ~ncomp,
  "e2i", 1,
  "i2o", 1,
