...
nfiles <- 10#config$nsimulations ## set to NULL or the actual number of sim files to include for final report
...
- **Current Measures Stopped April 26th**, the measures described in the Current Measures scenario are held up until April 26th as annonced on, then we assume a return to baseline, early epidemic level transmissibility for the foreseeable future.

- **Test and Isolate**, which assumes that current measures are replace on April 26th by a test and isolation, that shorten the infectious period of about half for about half of the individuals. This measure are maintained for the foreseeable future.

```{r rmd_formatting}
scen_desc_str <- "three scenarios with current measures unil April 26th followed by 1) a indefinite continuation of current measures, 2) a return to baseline transmissiblity, and 3) targeted testing and isolation"
fig_counter <- 1
tab_counter <- 1
```
...
```{r r0}
r0 <- readRDS("reports/R0_reduction.rds") %>%
  filter(ll_comp == "d-deltah", ShortName == "CH")
```


### Model assumptions
We built a stochastic, Susceptible-Exposed-Infected-Recovered (SEIR) model of SARS-CoV-2 transmission in canton Switzerland.
...
*Rates of Death and Hospitalization.* From our analysis of 391 confirmed SARS-CoV-2 infections in Shenzhen, China, we estimate the average time to hospitalization from symptom onset follows a log-normal distribution with median 3.42 days (IQR 2.01, 5.83) (Fig. 3). Other rates where derived from Vaud canton hospitalization data. We fitted a log normal distribution to the observed distribution of time hopsitalized, time in ICU, time from hospitalization to ICU and time to death. We estimate from current data that, among those hospitalized, 20% will be admitted to the ICU.
...
```{r sims and data, fig.width=10, fig.height=3.5}
national_data <- read_csv("reports/national_data.csv") %>% 
  select(date, cases, current_hosp, deaths, current_icu) %>% 
  rename(hosp = current_hosp,
         icu = current_icu) %>% 
...
make_combined_plot(quantiles_sims, national_data, time_cut, nrow =1)

capdata <- paste0("**Fig. ", fig_counter, "**: Simulation envelops of current scenario up to April 8th.")
fig_counter <- fig_counter + 1
```
...
We took parameter estimates on the effectiveness of interventions from Milwaukee in this study.
...
## Inference
We support the development of our planning scenarios with a modelling analysis of the epidemiological data available up to April 8th. We aim at inferring time-varying estimates $R_0$ at the national level following the progressive introduction of NPIs from March 2nd to March 20th.
