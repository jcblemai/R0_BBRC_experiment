...
- **R0 around 1.2**, which assumes that current measures are replaced from May 1 on by a combination of measures that keeps the $R_0$ around 1.2

- **R0 around 0.9**, which assumes that current measures are replaced from May 1 on by a combination of measures that keeps the $R_0$ around 0.9.



```{r rmd_formatting}
scen_desc_str <- "three scenarios with current measures until May 1st followed by 1) a return to baseline transmissiblity, 2) measures that keeps R0 around 1.2 and 3) measures that keeps R0 around 0.9."
fig_counter <- 1
tab_counter <- 1
```
