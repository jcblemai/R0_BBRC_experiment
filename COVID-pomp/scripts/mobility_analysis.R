...
getCorr <- function(x, y) {
  crosscorr <- ccf(x, y, lag.max = 14, plot = F, na.action = na.omit)
  crosscov <- ccf(x, y, lag.max = 14, plot = F, na.action = na.omit, type = "covariance")
...
crosscorrs <- joined %>% 
  filter(date.x >= "2020-02-28", date.x <= "2020-03-30") %>% 
  group_by(ShortName, var) %>% 
